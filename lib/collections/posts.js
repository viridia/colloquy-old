...
if (Meteor.isServer) {
  ...
  // Subscription for a single post, indexed by slug, with comments.
  Meteor.publish('postAndComments', function(postId) {
    const cursor = Posts.find({ slug: postId });
    if (cursor.count() == 1) {
      // Fetch comments too.
      const post = cursor.fetch()[0];
      return [cursor, Comments.find({ parentPost: post._id })];
    }
    return cursor;
  });
